...
ARG ARG_GO_MODULE_NAME="github.com/mkyc/m-post-infra-setup"
...
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w $(govvv -flags -pkg $GO_MODULE_NAME/cmd -version $M_VERSION)" -x -o /m-post-infra-setup $GO_MODULE_NAME
...
WORKDIR /workdir
ENTRYPOINT ["/workdir/m-post-infra-setup"]

COPY resources/project /resources/project
COPY --from=builder /m-post-infra-setup /workdir
